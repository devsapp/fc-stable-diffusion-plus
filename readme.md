...
使用serverless devs将stable-diffusion部署到阿里云函数计算上，支持自定义模型
...
选择直接部署-> 杭州/北京/上海/深圳地域 -> 复制开发者准备好的容器镜像
 杭州region: registry.cn-hangzhou.aliyuncs.com/serverlessdevshanxie/sd-auto-nas:v2
   北京region: registry.cn-beijing.aliyuncs.com/serverlessdevshanxie/sd-auto-nas:v2 
   深圳region: registry.cn-shenzhen.aliyuncs.com/serverlessdevshanxie/sd-auto-nas:v2 
   上海region: registry.cn-shanghai.aliyuncs.com/serverlessdevshanxie/sd-auto-nas:v2  
点击创建并部署默认环境
### 应用部署
...
其中sd开头的是主服务，目前因为没镜像还不能访问，
admin开头的是我们的管理后台，接下来我们需要先配置一下管理后台，然后把我们的模型上传上去
...
基于[https://github.com/AbdBarho/stable-diffusion-webui-docker/tree/master/services/AUTOMATIC1111](https://github.com/AbdBarho/stable-diffusion-webui-docker/tree/master/services/AUTOMATIC1111)这个项目，替换其中的**entrypoint.sh**
```shell
#!/bin/bash
